...
class WFADotExporterWithPartitionMapper(PDFAModelExportingStrategy):
    def __init__(self, partitioner: ProbabilityPartitioner, partition_mapper: PartitionMapper, pretty = False) -> None:
        super().__init__()
    ...
    def export(self, model, path=None, file_name=None):
        if self._pretty:
            graph = self.create_pretty_graph(model)
        else:
    ...
    def create_pretty_graph(self, model):            
        graph = Digraph('weighted_automaton', format='png')
        graph.attr(rankdir='LR', margin='0', size='15')
            ...
            for symbol, weighted_transitions in state.transitions_list.items():
                for weighted_transition in weighted_transitions:
                    ...
                    if (state_name, weighted_transition_name) in transitions.keys():
                        ...
                        transitions_symbols[(state_name, weighted_transition_name) ] = []
                        new = str(symbol) + "-" + transition_representation
                    transitions[(state_name, weighted_transition_name)] = new
                    transitions_symbols[(state_name, weighted_transition_name)].append(symbol)
            for key in transitions.keys():
                label = self._get_pretty_label(transitions_symbols[key])
                s_from, s_to = key
                graph.edge(s_from, s_to, label)
            
        return graph
        
    def _get_pretty_label(self, symbols):
        #Assuming they can be ordered
        if len(symbols)==1:
            return str(symbols[0])
        symbol_ints = [int(symbol.value) for symbol in symbols]
        symbol_ints.sort()
        max_val = max(symbol_ints)
        min_val = min(symbol_ints)
        pos = min_val+1
        last_min = min_val
        intervals = []
        while pos <= max_val:
            if pos in symbol_ints:
                pos+=1
            else:
                intervals.append((last_min, pos-1))
                while pos not in symbol_ints:
                    pos+=1
                last_min = pos
            if pos > max_val:
                intervals.append((last_min, pos-1))
