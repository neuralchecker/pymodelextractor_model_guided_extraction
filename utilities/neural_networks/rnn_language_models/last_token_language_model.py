...
#-Network was trained with a framework trainer (for example FrameworkLanguageModelTrainer)
class LastTokenLanguageModel(RNNLanguageModel):
    ...
    def sequence_probability(self, sequence: Sequence, debug = False):
        ...
        for i in range(len(word)):
            ...
            if debug: 
                ...
                print('Pred: ',yhat[i])
                print('Value in letter: ', yhat[i][symbol_index])
            result = result * yhat[i][symbol_index]    
            log_result = log_result + np.log(yhat[i][symbol_index])    
        return result, log_result

    def _to_categorical(self, sequence):
        return to_categorical(sequence, self._vocab_size-1)

    def _get_symbol_index(self, symbol):
        return np.argmax(to_categorical(self._encoder.encode(symbol)-1,num_classes=len(self.alphabet)+1))     

    def _get_encoded_symbol_index(self, encoded_symbol):
        return np.argmax(to_categorical(encoded_symbol-1,num_classes=len(self.alphabet)+1)) 
